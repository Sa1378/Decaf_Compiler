...
do
    ...
    javac -cp "java_cup_runtime.jar;java_cup.jar;jflex-full-1.8.2.jar;" Main.java Parser.java Sym.java Yylex.java
    if [ $? -eq 1 ]; then
        echo "Code did not Compiler"
    else
        echo "Core compiled successfuly"
        java -cp "java_cup_runtime.jar;java_cup.jar;jflex-full-1.8.2.jar;" Main -i $filelist -o $output_filename
        if [ $? -eq 0 ]; then
